      ...
      >
        ...
        <el-table-column fixed="right" label="操作" min-width="200">
          <template slot-scope="scope">
            ...
            <el-divider direction="vertical"></el-divider>
            <el-button @click="reset(scope.row.userId)" type="text" size="small"
              >重置</el-button
            >
          </template>
        </el-table-column>
      ...
      >
        ...
        <el-form :model="changeform" :rules="rules" ref="changeform">
          ...
          <el-form-item
            ...
            prop="cardId"
          >
            <el-input v-model="changeform.cardId" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item
            label="角色"
            :label-width="formLabelWidth"
            prop="region"
          >
            <el-select v-model="changeform.region" placeholder="请选择角色">
              <el-option
                v-for="item in typeOptions"
                :key="item.roleId"
                :label="item.roleName"
...
export default {
  ...
  methods: {
    ...
    deleteClick(val) {
      _userDelete({ id: val.userId }).then((res) => {
        if (res.code == 200) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          const data = {
            ...
            gender: this.changeform.sex == "女" ? "F" : "M",
            idCard: this.changeform.cardId,
            roleId: this.changeform.region,
