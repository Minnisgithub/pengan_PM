  ...
  <div>
    ...
    <div class="content-container content-drawer">
      ...
      >
        ...
        <el-table-column prop="roleUserNum" label="当前关联人数" width="200">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteClick(val) {
      _roleDelete({ id: val.roleId }).then((res) => {
        if (res.code == 200) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          const data = {
            ...
            permissionList: this.newPermissionList,
          };
    ...
    changeClick(val) {
      ...
      this.checkedKeyList = [];
      for (const iterator of val.permissionList) {
        if (iterator.child.length > 0) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          const data = {
            ...
            permissionList: this.checkPermissionList,
          };
    ...
    handleCheckChange(e, data) {
      let list = [...data.checkedNodes,...data.halfCheckedNodes]
      this.newPermissionList = list;
      console.log(list);
